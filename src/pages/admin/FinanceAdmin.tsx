...
import { TrendingUp, TrendingDown, DollarSign, Wallet, Search, Loader2 } from 'lucide-react';
...
export default function FinanceAdmin() {
  ...
  const { lineChartData, pieChartData } = useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      // Date grouping
      if (!byDate[dateStr]) {
        byDate[dateStr] = { date: dateStr, income: 0, expense: 0 };
      }
      if (t.type === 'income') byDate[dateStr].income += amount;
      if (t.type === 'expense') byDate[dateStr].expense += amount;
